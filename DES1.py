...
class MyDES:
    ...
    @staticmethod
    def _bit_encode(s: str) -> str: #Из строки в двоичный код
        """
                 Преобразуйте строку в строковую форму 01
                 bitarray(a) - создает битовый массив размером a
                 join()-метод списка(объединяет)
                 bin() - преобразует целое число в двоичный формат
                 hex() - преобразует целое число в шестнадцатиричную строку с префиксом 0x
                 Байты - это кодировка. Чтобы получить текст нужно расшифровать(encode())
                 to01() возвращает строку из битовых эл-ов
                 int(str(число), b(с.и.)) - возвращает значение этого числа
                 На выходе массив из битов открытого текста
        """
        return bitarray(
    ...
    @staticmethod
    def _replace_block(block: str, replace_table: tuple) -> str: #Метод преобразований
        """
    ...
    def _processing_encode_input(self, enter: str) -> list: #Разделение текста на
        """
                 Преобразуйте входную строку в двоичную форму и разделите ее на группу без 64 ​​бит
                 #range() - возвращает список с прогрессией [1, 2, ...]
        """
        result = []
    ...
    def _init_replace_block(self, block: str): #IP
        """
    ...
    @staticmethod
    def _block_extend(block: str) -> str: #3.1.1 Расширенная замена
        """
                 Расширенная замена
        """
        extended_block = ""
    ...
    def _s_box_compression(self, num: int, block48: str) -> str: #Замена
        """
    ...
    def _f_function(self, right: str, is_decode: bool, num: int):
        right = self._block_extend(right) #Расширенная замена (на выходе 48 битов и деление на 8 блоков по 4 и расширение до 6)
        if is_decode:
            sbc_result = self._s_box_compression(15 - num, right) #Замена S-блоком
        else:
            sbc_result = self._s_box_compression(num, right)
        # print (f "Сжатый результат блока s: {sbc_result}")
        return self._p_box_replacement(sbc_result) #Замена коробки Р

    def _iteration(self, block: str, key: str, is_decode: bool) -> str: #Делит и объединяет блоки
...
if __name__ == '__main__':
    #key = "hahahha"
    md = MyDES()
    #file = open('shifr.txt', 'r' (w!))
    #print(file.readline()) - считывает строку целиком
    # for row in file:
    #   print(row) - читывает все строки по очереди
    #file.write('hello\n') - Запись в файл
    #file.close()

    #print("Зашифрованные данные DES:" + md.encode("jutiyuiop", key))
    #print(f"Расшифрованные данные:" + md.decode("0xbe6bd189b78e6c840x47274f078cdce00b", key))